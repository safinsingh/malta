	...
	match opts.subcmd {
		SubCommand::Encrypt => {
			let raw = fs::read_to_string(opts.config)
				.expect("There was an error reading the config");
			crypto::compress(raw);
		}
		SubCommand::GenKey => {
		...
		SubCommand::Score => {
			...
			if let Ok(id) = fs::read_to_string("TeamID") {
				...
				if let Err(e) =
					client.post(&config.remote).json(&params).send()
				{
					println!("{}", e);
				}
			} else {
				panic!(
					"Failed to read TeamID, didn't send scoring data to \
					 remote endpoint"
				);
			}
		}
